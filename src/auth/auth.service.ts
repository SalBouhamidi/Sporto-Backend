...
export class AuthService {
    ...
    verifyToken(token: string) {
        try {
            const verification = validateJwt(token);
            // console.log('ehehehehehhe', verification)
            if (!verification) {
                return "your token has been expired or destroy, try to login again"
            }
            return verification
